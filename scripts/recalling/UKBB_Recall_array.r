##===============================
##This is a script for recalling per array without spliting per batch
##
...
snps<-read.table("/home/ey246/UKBB_recalling/by_array.txt", as.is=T, header=F)
...
ifelse(file.exists(f1) & file.exists(f2), ar<-c("UKBB", "UKBL"), ifelse (file.exists(f1), ar<-"UKBB", ar<-"UKBBL"))
...
dsn=dplyr::filter(dsn, !grepl('id1|id2|...|idn',ID))


#load("/home/ey246/claudia/Intensities_UKBBAffx.89025742array_UKBB.RData")
#i="Affx.89025742"
##================================
...
plot.name <- paste("ukbb_",variant.name,"_array_",j,sep="")
...
define.happy <- FALSE
     ...
     # plot(intensities.a, intensities.b, 
      ...
      write.table(paste(calls.genotypes,collapse=" "),paste0('./recalled/',"ukbb",'_',variant.name,"array_",j,'.tped'),col.names=FALSE,row.names=FALSE,quote=FALSE,sep="\t")
      write.table(calls.fam,paste0('./recalled/',"ukbb",'_',variant.name,"array_",j,'.tfam'),col.names=FALSE,row.names=FALSE,quote=FALSE,sep="\t")
#write.table(paste(calls.genotypes,collapse=" "),paste0('/home/ey246/UKBB_recalling/redone/',"ukbb",'_',variant.name,"array_",j,'.tped'),col.names=FALSE,row.names=FALSE,quote=FALSE,sep="\t")
#write.table(calls.fam,paste0('/home/ey246/UKBB_recalling/redone/',"ukbb",'_',variant.name,"array_",j,'.tfam'),col.names=FALSE,row.names=FALSE,quote=FALSE,sep="\t")
      obj.list <- ls()
      dev.off()
}}     