...
bp<- ggplot(df, aes(x,y))+ geom_point(aes(color=Legend),shape=1,size=8) + scale_color_manual(Legend,values=clist,name=t) 
#bp<- bp + facet_grid(. ~ Tag,scale='free')
bp<- bp + labs(x = xlabel, y = ylabel, title = t)
...
dat_text <- data.frame(
	label = c(paste("N = 178 \n Rho = 0.9 \n P < 2.2e-16")),
	Tag = c("Genotyped SNVs"))


bp<- bp + geom_text(data = dat_text,mapping = aes(x = c(2), y = c(3), label = label),fontface = "bold",size = 3,family="Helvetica",color="black")  

bp + geom_abline(slope=1, intercept=0,linetype='F1',colour='gray29') + theme_classic(base_size=20)
