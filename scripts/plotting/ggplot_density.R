...
dp<- ggplot(df1, aes(x=x1,fill=Legend)) + geom_density(alpha=.7)+ scale_fill_manual(Legend,values=clist,name=t1)+ theme_classic()
dp<- dp + labs(x = xlabel1, y = ylabel1) +  
	theme(plot.title = element_text(hjust = 0.5), 
	text = element_text(size=14),
	legend.position="bottom",
	axis.text.x = element_text(angle = 0, family="Helvetica",face="bold",size=12),
...
dp + annotation_custom(ggplotGrob(bp),xmin = -1, ymin = 0.2, xmax = -0.2)
